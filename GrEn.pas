...
var
  ...
  R,G,B: LongInt;
begin
  for CurPl:= 0 to AOb.PolygoneNum - 1 do begin
    ...
    if Dp > 0 then
      begin
        ...
        if Inten < 0 then Inten := 0;
        Inten := Inten + AmL;
        if Inten > 31 then Inten := 31;
...
begin
  for Y := 0 to 479 do begin
    if YBt[Y].StartX = -16000 then Continue;
...
var
  ...
  Temp: TPolygon;
begin
  for Y:= 0 to Object3D.PolygoneNum - 1 do begin
  ...
  for X := 0 to Object3D.PolygoneNum - 1 do
    for Y := 0 to Object3D.PolygoneNum - 2 do
      if Object3D.PolygoneStor[Y].AverageZ >
         Object3D.PolygoneStor[Y + 1].AverageZ then
        begin
  ...
  case RenderMode of
    rmWframe:
      begin
        ...
        for X := 0 to Object3D.PolygoneNum - 1 do
          with Object3D.PolygoneStor[x] do begin
            DrawLine3D(Point[0].X, Point[0].Y, Point[0].Z, Point[1].X, Point[1].Y, Point[1].Z);
            DrawLine3D(Point[1].X, Point[1].Y, Point[1].Z, Point[2].X, Point[2].Y, Point[2].Z);
            if PointsNum = 3 then
              DrawLine3D(Point[2].X, Point[2].Y, Point[2].Z, Point[0].X, Point[0].Y, Point[0].Z)
            else
              begin
                DrawLine3D(Point[2].X, Point[2].Y, Point[2].Z, Point[3].X, Point[3].Y, Point[3].Z);
                DrawLine3D(Point[3].X, Point[3].Y, Point[3].Z, Point[0].X, Point[0].Y, Point[0].Z);
              end;
          end;
      end;
    rmSld:
    ...
    rmSldShade:
      begin
        ...
        for X := 0 to Object3D.PolygoneNum - 1 do
          with Object3D.PolygoneStor[X] do begin
            if not Object3D.PolygoneStor[X].Visible then Continue;
...
begin
  for P:=0 to Object3D.PolygoneNum - 1 do
    with Object3D.PolygoneStor[P] do begin
      if Z <> 0 then
        for I:= 0 to PointsNum - 1 do begin
...
begin
  ...
  Length := VMgn(Result);
  if Length = 0 then Length := 0.00001;
